...
public class MnemonicUtils {
     ...
     */
    public static String generateMnemonic(Context context, byte[] initialEntropy) {
        return generateMnemonic(context, initialEntropy, Locale.ENGLISH);
    }

    public static String generateMnemonic(Context context, byte[] initialEntropy, Locale locale) {
        WORD_LIST = populateWordList(context, locale);
     ...
     */
    public static byte[] generateEntropy(Context context, String mnemonic) {
        WORD_LIST = populateWordList(context);
    ...
    public static boolean validateMnemonic(Context context, String mnemonic) {
        return validateMnemonic(context, mnemonic, Locale.ENGLISH)
                || validateMnemonic(context, mnemonic, Locale.CHINESE)
                || validateMnemonic(context, mnemonic, Locale.TAIWAN);
    }

    private static boolean validateMnemonic(Context context, String mnemonic, Locale locale) {
        if (isMnemonicEmpty(mnemonic)) {
            return false;
        }
        try {
            WORD_LIST = populateWordList(context, locale);
            mnemonicToBits(mnemonic);
            return true;
        } catch (Exception ex) {
            return false;
        }
    ...
    private static List<String> populateWordList(Context context) {
        try {
            InputStream inputStream = context.getAssets().open("mnemonic/en-mnemonic-word-list.txt");
    ...
    private static List<String> populateWordList(Context context, Locale locale) {
        String fileName = "mnemonic/en-mnemonic-word-list.txt";
        if (Locale.CHINESE.equals(locale)) {
            fileName = "mnemonic/chinese_simplified.txt";
        } else if (Locale.TAIWAN.equals(locale)) {
            fileName = "mnemonic/chinese_traditional.txt";
        }
        try {
