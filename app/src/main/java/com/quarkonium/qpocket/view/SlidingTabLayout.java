...
import com.quarkonium.qpocket.view.listener.OnTabSelectListener;
...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private int mHeight;
    private boolean mEndHasMargin;

    private boolean mIsAlignParent = false;

    private boolean mNoAnimScroll = false;
    ...
    private void addTab(final int position, String title, View tabView) {
        ...
        tabView.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (ToolUtils.isFastDoubleClick(300)) {
                    return;
                }
        ...
        if (mEndHasMargin && position == mTabCount - 1) {
            lp_tab.rightMargin = (int) UiUtils.dpToPixel(8);
        }
        if (mTabWidth > 0) {
...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
            ...
            if (tv_tab_title != null) {
                ...
                if (!mTabSpaceEqual && mIsAlignParent) {
                    if (i == 0) {
                        tv_tab_title.setPadding(0, 0, (int) mTabPadding, 0);
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        if (mScrollState == ViewPager.SCROLL_STATE_IDLE) {
            if (mNoAnimScroll) {
                scrollToCurrentTab(0);
            } else {
                scrollToCurrentTab(250);
            }
            mNoAnimScroll = false;
        }
    }
     ...
     */
    private void scrollToCurrentTab(long time) {
        if (mTabCount <= 0) {
            return;
        }
        scrollPosition(mCurrentTab, time);
    }


    public void scrollPosition(int position, long time) {
        ...
        int x = 0;
        int lastX = getViewLength(position - 1);//上一个
        if (lastX < nowX) {
            x = lastX - nowX;
        }

        int nextX = getViewLength(position + 2);//下一个
        if (nextX > nowX + getWidth()) {
            x = nextX - (nowX + getWidth());
        }
    ...
    //获取指定view在布局中得总长度
    private int getViewLength(int position) {
        if (position < 0) {
            position = 0;
        }
        ...
        for (int i = 0; i < position; i++) {
            length += mTabsContainer.getChildAt(i).getWidth();
        }
        return length;
    }
    ...
    private void calcIndicatorRect() {
        ...
        if (this.mCurrentTab < mTabCount - 1) {
            ...
            if (!mTabSpaceEqual && mIsAlignParent) {
                if (mCurrentTab == 0) {
                    nextTabLeft = nextTabLeft + margin * 2;
                    nextTabRight = nextTabRight + margin * 2;
                } else if (mCurrentTab == mTabCount - 2) {
                    nextTabLeft = nextTabLeft + margin / 2;
                    nextTabRight = nextTabRight + margin / 2;
                }
            }
        ...
        if (mIndicatorStyle == STYLE_NORMAL && mIndicatorWidthEqualTitle) {
            if (!mTabSpaceEqual && mIsAlignParent) {
                if (this.mCurrentTab == 0) {
                    mIndicatorRect.left = (int) (left - 1);
                    mIndicatorRect.right = (int) (right - margin * 2 - 1);
                } else if (this.mCurrentTab == mTabCount - 1) {
                    mIndicatorRect.left = (int) (left + margin * 2 - 1);
                    mIndicatorRect.right = (int) (right - 1);
                } else {
    ...
    public void setEndHasMargin(boolean hasMargin) {
        mEndHasMargin = hasMargin;
    }

    public void setIsAlignParent(boolean mIsAlignParent) {
        this.mIsAlignParent = mIsAlignParent;
    }
}
