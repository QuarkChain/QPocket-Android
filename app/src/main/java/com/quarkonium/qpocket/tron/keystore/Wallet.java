package com.quarkonium.qpocket.tron.keystore;

import android.text.TextUtils;

import com.quarkonium.qpocket.api.StandardCharsets;
...
public class Wallet {
    ...
    private static WalletFile createWalletFile(
        ...
        walletFile.setAddress(TronWalletClient.encode58Check(ecKeyPair.getAddress()));
    ...
    private static byte[] generateDerivedScryptKey(
        ...
        throw new CipherException("Unsupported prf:");
    }
    ...
    public static ECKey decrypt(String password, WalletFile walletFile)
        ...
        return decrypt(password, crypto);
    }

    public static ECKey decrypt(String password, String keyStore) throws CipherException {
        if (TextUtils.isEmpty(keyStore)) {
            throw new CipherException("keyStore is not null");
        }
        ...
        try {
            ...
            if (jsonObject.has("crypto")) {
                JSONObject cryptoObject = jsonObject.getJSONObject("crypto");

                //1。cipher
                String cipher = cryptoObject.getString("cipher");
                crypto.setCipher(cipher);

                String ciphertext = cryptoObject.getString("ciphertext");
                crypto.setCiphertext(ciphertext);

                //1。1cipherParams类
                JSONObject cipherParamsObject = cryptoObject.getJSONObject("cipherparams");
                String iv = cipherParamsObject.getString("iv");
                ...
                //2。cipherParams解析
                //cipherParams类型
                String kdf = cryptoObject.getString("kdf");
                crypto.setKdf(kdf);

                JSONObject kdfparamsObject = cryptoObject.getJSONObject("kdfparams");
                if (Wallet.SCRYPT.equals(kdf)) {
                    int dklen = kdfparamsObject.has("dklen") ? kdfparamsObject.getInt("dklen") : DKLEN;
                    int n = kdfparamsObject.getInt("n");
                    int p = kdfparamsObject.getInt("p");
                    int r = kdfparamsObject.has("r") ? kdfparamsObject.getInt("r") : R;
                    String salt = kdfparamsObject.getString("salt");
                    ...
                    kdfParams.setP(p);
                    kdfParams.setR(r);
                    kdfParams.setSalt(Numeric.prependHexPrefix(salt));

                    crypto.setKdfparams(kdfParams);
                } else if (Wallet.AES_128_CTR.equals(kdf)) {
                    int dklen = kdfparamsObject.has("dklen") ? kdfparamsObject.getInt("dklen") : DKLEN;
                    int c = kdfparamsObject.getInt("c");
                    String prf = kdfparamsObject.getString("prf");
                    String salt = kdfparamsObject.getString("salt");
                ...
                //3。mac
                String mac = cryptoObject.getString("mac");
                crypto.setMac(mac);
            }
    ...
    private static ECKey decrypt(String password, WalletFile.Crypto crypto) throws CipherException {
